...
## 🚀 Project Journey (Sensor Selection)
...
```
  ...
  * **The Math:** In `main.c`, `TIM1` is configured with a `Prescaler` of 72-1 and a `Period` of 20000-1.
      * `Prescaler = 72-1`: Divides the 72MHz system clock by 72, resulting in a 1MHz timer frequency. This means each "tick" is 1 microsecond (1µs).
      * `Period = 20000-1`: The timer counts from 0 to 19999, giving a total period of 20,000 µs, which is 20 milliseconds (ms). This is the standard 50Hz frequency for servos.
  * **The Formula:** `pulse = 500 + (angle / 180.0f) * 2000.0f` linearly maps an angle from 0-180° to a pulse width between 500µs (for 0°) and 2500µs (for 180°).
  * **The PID Connection:** The line `float servo_angle = 90.0f + output;` in the main loop takes the PID output (e.g., -20) and applies it as a correction to the servo's neutral position (90°), resulting in a new target angle (e.g., 70°).
